  ...
  gallery.innerHTML = images.map((image) => {
    ...
    return `
      <article class="work-card">
        <button class="work-media" type="button" data-full-image="${image.download_url}" data-title="${title}" aria-label="${title} 확대 보기">
...
const loadWorksImages = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("image directory request failed");
    }
